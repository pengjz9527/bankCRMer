# 徽小助 Design System v3.1

> 设计系统规范文档 v3.1，基于 v3.0 改进：
> - 核心聚焦两大任务：**智能日程**（横向滑动卡片 + AI排程）+ **商机看板**（目标→缺口→挖掘→新客闭环）
> - 待办卡片支持横向滑动浏览，同类型可折叠
> - 商机挖掘与考核目标关联，含转化率计算和新客拓展路径
> 所有后续页面开发以此为唯一参考。
> **原型参考**：`prototype/workbench.html`（原始原型）
> **工程实现**：`frontend/src/`（Vue 3 + Vite + Vant 4 生产代码）
> **版本**：v3.1 | **日期**：2026-07-27

---

## 〇、工程实现映射

> 以下章节描述的每个设计规范，在 `frontend/src/` 中均有对应的 Vue 3 实现。此映射表帮助从设计文档快速定位源码。

### 组件规范 → Vue SFC 映射

| 设计规范章节 | 规范名称 | Vue 组件文件 | 说明 |
|:----------:|---------|-------------|------|
| §4.1 | 头部 Header | `components/common/AppHeader.vue` | 白底 + 1px 分割线 + AI 呼吸灯 |
| §4.2 | AI 智能洞察卡片 | `components/business/AiSummaryBanner.vue` | 紫色渐变背景 + 发光投影 |
| §4.3 | AI 快捷指令 | `views/HomeView.vue` (内联) | 2×2 网格快捷入口 |
| §4.4 | 浮动 AI 按钮 | `components/business/FabButton.vue` | 右下角悬浮 + 浮动动画 |
| §4.5 | AI 标注标签 | `components/business/TaskCard.vue` (内联) | `.ai-label` 样式 |
| §4.6 | 卡片 Card | 全局 `.card` class | 白色背景 + 16px 圆角 + 投影 |
| §4.7 | 按钮 Button | Vant `van-button` | 主按钮 / CTA / 幽灵 / 描边 |
| §4.8 | 标签 Tag | 全局 `.tag` class | 5 色语义标签 |
| §4.9 | 产品卡片 | `components/business/ProductCard.vue` | 收益率红色 + 代销标签 |
| §4.10 | 进度条 | `views/PerformanceView.vue` (内联) | 8px 高 + 品牌色渐变 |
| §4.11 | 客户卡片 | `components/business/CustomerCard.vue` | 头像 + 等级 + AUM + 标签 |
| §4.12 | 商机卡片 | `components/business/OppBoardCard.vue` | 目标→缺口→挖掘闭环 |

### Store 数据映射

| Store 模块 | 文件 | 数据内容 |
|-----------|------|---------|
| `useAppStore` | `stores/app.ts` | Toast 提示、API 可用状态 |
| `useManagerStore` | `stores/manager.ts` | 客户经理姓名/头像/业绩 |
| `useScheduleStore` | `stores/schedule.ts` | 今日日程（6 类任务 + 6 张卡片） |
| `useOpportunityStore` | `stores/opportunity.ts` | 商机看板（目标/缺口/挖掘/新客） |
| `useCustomerStore` | `stores/customer.ts` | 客户列表（7 存量 + 3 新客 + 筛选） |
| `useProductStore` | `stores/product.ts` | 产品库（12 款产品 + 筛选/对比） |
...
|------|---------|------------|
| `/` | 工作台首页 | §十一 W1 工作台首页 |
| `/schedule/all` | 全部日程 | W2 日程处理 |
| `/opportunity` | 商机列表 | W3 商机管理 |
| `/customer` | 客户列表 | W8 客户管理 |
| `/product` | 产品搜索 | W5 产品搜索 |
| `/ai/chat` | AI 对话 | AI 全局助手 |
| `/battle-package/:id` | 作战包 | P 作战包与面谈 |
